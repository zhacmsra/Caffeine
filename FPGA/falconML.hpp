...
void ConvCore(bool exec, bool mode, data_t in[HWFIn_][HWFinR_][HWFinC_], data_t Cout[HWFOut_][HWFR_][HWFC_], wght_t weight[HWFOut_][HWFIn_][HWKsize_][HWKsize_], int oo, int ii, int rr, int frow, int fcol, int prow, int Ksize, int Kstride, wght_t Bias[HWFOut_][HWFR_*HWFC_]) {
#pragma HLS inline off
...
#pragma HLS ARRAY_PARTITION variable=tmp complete dim=1
    ...
    if(ii==0){
        ...
        for(int rb=0; rb<prow; rb++){
            ...
            #pragma HLS pipeline
		if((mode==CONV))
		    baddr = oo/HWFOut_; //if((rb==0)&&(rc==0))
		else
		    baddr = prow*fcol + rc;

                for(int o=0; o<HWout_; o++){
                #pragma HLS UNROLL 
		    Cout[o][rb][rc] = Bias[o][baddr];
                }
            }
        }
    }
...
#pragma HLS ARRAY_PARTITION variable=weight complete dim=2
    ...
    bw_t btmp;
    wght_t Bias[HWFOut_][HWFR_*HWFC_];
    ...
    //optimization: load CONV&FCN bias with one piece of hardware
    load_bias:
    for(int rrbc=0; rrbc<floop; rrbc++){
    #pragma HLS pipeline
        btmp = m_fm[bias_addr_t+ rrbc];
        for(int o=0; o<HWout_; o++){
        #pragma HLS UNROLL 
            ap_uint<DSIZE> ibtmp = btmp.range((o+1)*DSIZE-1, DSIZE*o);
            if_wght_t *fbtmp = (if_wght_t*)&ibtmp;
            if_wght_t biasdata = *fbtmp;
            Bias[o][rrbc] = (wght_t)(biasdata); 
        }
    } 
/*
    if((mode==CONV)) {
	...
	#pragma HLS pipeline
	    btmp = m_fm[bias_addr_t+ oo/HWout_ ];
	    for(int o=0; o<HWout_; o++){
	    #pragma HLS UNROLL 
	        ap_uint<DSIZE> ibtmp = btmp.range((o+1)*DSIZE-1, DSIZE*o);
	        if_wght_t *fbtmp = (if_wght_t*)&ibtmp;
	        if_wght_t biasdata = *fbtmp;
	        //give bias
	        Bias[o][oo/HWout_] = (wght_t)(biasdata); 
	    }
	}
    }
    else {
	int rb=0,  rc=0;
	load_bias_fcn:
	for(int rrbc=0; rrbc<frow*fcol; rrbc++){
	    #pragma HLS pipeline
		btmp = m_fm[bias_addr_t+ rrbc];
	        for(int o=0; o<HWout_; o++){
	        #pragma HLS UNROLL 
            ...
            prow = (prow_factor>=(int)HWFR_)?(int)HWFR_:prow_factor;
        	    for(ii=0; ii<fin+HWin_; ii=ii+HWin_){
        	        ...
        	        if(pinpon_flag_in == 1) {
        	            ...
        	            ((flag_con), mode, in_1, Cout, weight_1, oo, (ii-HWin_), rr, frow, fcol, prow, Ksize, Kstride, Bias); 
        	        }
        	        else {
        	            ...
        	            ConvCore<data_t, HWFIn_, HWFOut_, HWFR_, HWFC_, HWFinR_, HWFinC_, HWKsize_, HWin_, HWout_>
        	            ((flag_con), mode, in, Cout, weight, oo, (ii-HWin_), rr, frow, fcol, prow, Ksize, Kstride, Bias); 
        	        }
