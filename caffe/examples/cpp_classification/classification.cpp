...
int main(int argc, char** argv) {
  ...
  while(std::getline(out, file)){
    ...
    //std::cout << "Display the image under test:" << std::endl;
    //cv::Mat image = cv::imread(file);
    //cv::namedWindow("Picture Under Test");
    //cv::imshow("Picture Under Test", image);
    //cv::waitKey(1000);
    num++;
    IplImage *pImgC = cvLoadImage(file.c_str());
    cvNamedWindow("Image Under Test");
    cvMoveWindow("Image Under Test", 320 - (pImgC->width / 2), 384 - (pImgC->height / 2));
    ...
    //insert your code here
    //IplImage *pImgG = cvLoadImage(file.c_str(), 0);
    IplImage *pImgG;
    ...
    strID = "Execution Time: " + ConvertFloat(get_time(start, end)) + " ms";
    cvPutText(pImgC, strID.c_str(), cvPoint(P.x, P.y+predictions.size()*25+2), &font, CV_RGB(255,0,0));
    cvNamedWindow("Image Recognition Result on CPU");
    cvMoveWindow("Image Recognition Result on CPU", 320 - (pImgG->width / 2), 384 - (pImgG->height / 2));
    cvShowImage("Image Recognition Result on CPU", pImgC);
    if ( num % 5 == 0 ) {
        c = cvWaitKey(0);
        gettimeofday(&synt, NULL);
    ...
    cvDestroyWindow("Image Recognition Result on CPU");
